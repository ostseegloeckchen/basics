    ...
    def run(self):
        while not self.w.kollision(self):
            ...
            if self.w.gelandet():
                ...
                break
        else:   #Warum gehört das "else" zu "while not" und nicht zu "if"?
            self.c.create_text(100, 50,
                                text = "Schade, verloren!",
                                font = ("Times New Roman", 20), fill = "white")

    def steuere(self, event):
        if   event.keysym_num == 65362: self.vy -= 0.5    # hoch
        elif event.keysym_num == 65364: self.vy += 0.5  # runter
        elif event.keysym_num == 65363: self.vx += 0.5  # rechts
        elif event.keysym_num == 65361: self.vx -= 0.5  # links
